...
SECTION_GROUPS = (
    ("Сток", (S.STOCK_BALANCES, S.STOCK_TOTAL, S.STOCK_SUPPLIES, S.STOCK_INBOUND, S.STOCK_RANDOMIZER, S.STOCK_COST_REPORT, S.STOCK_OPERATIONS)),
...
SECTION_PARENTS = {
    ...
    **{section: S.UNIT_ECONOMICS_1C for section in (S.UNIT_ECONOMICS_WB, S.UNIT_ECONOMICS_OZON, S.UNIT_ECONOMICS_YANDEX, S.REPORT_UNIT_PROFIT, S.REPORT_TARGET_PRICE)},
}
...
def section_for_path(path: str) -> SectionName | None:
    ...
    for prefix, section in (
        ("/stock/total", S.STOCK_TOTAL), ("/stock/supplies", S.STOCK_SUPPLIES),
        ("/stock/planning", S.STOCK_SUPPLIES), ("/stock/inbound", S.STOCK_INBOUND),
        ("/stock/randomizer", S.STOCK_RANDOMIZER), ("/stock/cost-report", S.STOCK_COST_REPORT),
        ...
        ("/ai-agents", S.AI_AGENTS), ("/api/ai-agents", S.AI_AGENTS),
        ("/admin/google-export", S.ADMIN_GOOGLE_EXPORT),
        ("/admin/integrations", S.ADMIN_INTEGRATIONS), ("/api/admin/integrations", S.ADMIN_INTEGRATIONS),
...
def _legacy_level(user: User, section: SectionName) -> SectionAccessLevel:
    if user.access_profile is not None:
        ...
        if "WB" not in markets:
            return L.READ if "YANDEX MARKET" in markets and profile_has_permission(user, ActionPermission.UNIT_ECONOMICS_VIEW) else L.NONE
...
def access_limit(user: User | None, section: SectionName) -> SectionAccessLevel:
    ...
    if user.access_profile is not None or user.access_scopes:
        ...
        if marketplace and marketplace not in accessible_marketplaces(user):
            return L.NONE
        if user.access_profile is not None and (section in SECTION_GROUPS[1][1] or section in SECTION_GROUPS[2][1]):
...
def default_access_level(user: User, section: SectionName) -> SectionAccessLevel:
    if section is S.AI_AGENTS:
        return L.WRITE if any(has_access(user, item) for _, group in SECTION_GROUPS[:3] for item in group) else L.NONE
...
def active_section(active: str) -> SectionName | None:
    return {
        "unit_1c_settings": S.UNIT_ECONOMICS_WB, "unit_1c_wb": S.UNIT_ECONOMICS_WB,
        "unit_1c_ozon": S.UNIT_ECONOMICS_OZON, "unit_1c_yandex": S.UNIT_ECONOMICS_YANDEX,
        "unit_1c_reports": S.REPORT_UNIT_PROFIT, "unit_1c_target_price": S.REPORT_TARGET_PRICE,
        "stock": S.STOCK_BALANCES, "stock_total": S.STOCK_TOTAL, "stock_supplies": S.STOCK_SUPPLIES,
        "stock_inbound": S.STOCK_INBOUND, "stock_randomizer": S.STOCK_RANDOMIZER,
        "stock_cost_report": S.STOCK_COST_REPORT, "stock_operations": S.STOCK_OPERATIONS, "ai_agents": S.AI_AGENTS,
        "admin": S.ADMIN_USERS,
        "admin_google_export": S.ADMIN_GOOGLE_EXPORT, "admin_integrations": S.ADMIN_INTEGRATIONS,
    }.get(active)
