...
def validate_settings(settings: StockSheetExportSettings) -> None:
    ...
    for marketplace in repository.MARKETPLACES:
        has_configured_target = any(
            target.sheet_name.strip() for target in settings.targets if target.marketplace == marketplace
        )
...
def _combined_stock_snapshot(
    ...
    *,
    ...
    for store_slug in store_slugs:
        ...
        warnings.extend(
            f"{STORES[store_slug].name} · {marketplace}: {warning}" for warning in inbound.warnings
        )
...
def export_store(
    ...
    *,
    ...
    for current_marketplace in selected_marketplaces:
        targets = tuple(target for target in settings.targets if target.marketplace == current_marketplace)
        if not targets:
            continue
