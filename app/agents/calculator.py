...
INPUT_LABELS = {
    "retail": "Цена без СПП, руб",
    "client": "Цена с СПП, руб",
    "wallet": "Цена с WB Кошельком, руб",
    "spp": "СПП, %",
    "walletPercent": "WB Кошелёк, %",
    "commission": "Комиссия WB, %",
    "commissionRub": "Комиссия WB, руб",
    "drr": "ДРР с выкупом, % (7 дней)",
    "buyoutPercent": "Процент выкупа",
    "advertisingRub": "Рекламные расходы на единицу, руб",
    "logistics": "Логистика, руб",
    "storage": "Ставка хранения, руб/день",
    "storageTotal": "Хранение, руб",
    "acquiringPercent": "Эквайринг, %",
    "acquiringRub": "Эквайринг, руб",
    "purchase": "Закупочная стоимость, руб",
    "team": "Комиссия компании, %",
    "teamRub": "Комиссия компании, руб",
    "fulfillment": "Затраты на ФФ, руб",
    "vat": "НДС, %",
    "vatRub": "Налог НДС, руб",
    "usn": "УСН, %",
    "osno": "ОСНО, %",
    "secondaryTaxRub": "Налог УСН/ОСНО, руб",
}
...
def calculator(product):
    ...
    tax_system = (
        "osno" if product.get("store_slug") == "gogol" and details.get("tax_system") == "osno" else "usn"
    )
    wallet_percent = None
    if client is not None and client > 0 and wallet is not None and wallet > 0:
        wallet_percent = next(
            (p for p in range(100) if client - math.ceil(client * p / 100) == math.floor(wallet + 0.5)), None
        )
    ...
    values = {
        ...
        "storage": storage,
        "storageTotal": number(details.get("storage_sum"), mul(storage, days)),
        "acquiringPercent": acquiring,
        "acquiringRub": mul(retail, acquiring, 100),
        "purchase": number(details.get("purchase_cost")),
        "team": team,
        "teamRub": mul(retail, team, 100),
        "fulfillment": number(details.get("fulfillment_cost")),
        "vat": vat,
        "vatRub": number(details.get("vat_value"), mul(client, vat, 100 + vat) if vat is not None else None),
        "usn": number(details.get("usn_percent")),
        "osno": number(details.get("osno_percent")),
        "secondaryTaxRub": number(details.get(tax_system + "_value")),
    }
    values = {k: math.floor(v * 100 + 0.5) / 100 if v is not None else None for k, v in values.items()}
    ...
    vat = number(
        values["vatRub"],
        mul(client, values["vat"], 100 + values["vat"]) if values["vat"] is not None else None,
    )
    secondary = number(
        values["secondaryTaxRub"],
        mul(client if tax_system == "osno" else subtract(client, vat), values[tax_system], 100),
    )
    ...
    acquiring = number(
        values["acquiringRub"],
        mul(current, number(values["acquiringPercent"], number(details.get("acquiring"))), 100),
    )
    ...
    results = {
        ...
        "storage": storage,
        "vat": vat,
        "secondaryTax": secondary,
        "secondaryTaxLabel": "ОСНО" if tax_system == "osno" else "УСН",
        "tax": tax,
        "advertising": values["advertisingRub"],
        "margin": margin,
        "roi": margin / purchase * 100
        if margin is not None and purchase is not None and purchase > 0
        else None,
    }
    return {
        ...
        "result_labels": {
            "current": "Цена без СПП, руб",
            "sppPrice": "Цена с СПП, руб",
            "walletPrice": "Цена с WB Кошельком, руб",
            "netRevenue": "Выручка после расходов WB и рекламы на единицу, руб",
            "purchase": "Закупочная стоимость, руб",
            "fulfillment": "ФФ, руб",
            "acquiring": "Эквайринг, руб",
            "commission": "Комиссия WB, руб",
            "teamCommission": "Комиссия компании, руб",
            "logistics": "Логистика, руб",
            "storage": "Хранение, руб",
            "vat": "НДС, руб",
            "secondaryTax": "УСН/ОСНО, руб",
            "secondaryTaxLabel": "Название налога",
            "tax": "Все налоги, руб",
            "advertising": "Реклама на единицу, руб",
            "margin": "Чистая прибыль на единицу, руб",
            "roi": "ROI калькулятора, %",
            "target_price": "Целевая цена с WB Кошельком, руб",
        },
