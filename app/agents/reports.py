...
TAG_LABELS = {
    "goal_week": "Цель неделя",
    "goal_day": "Цель день",
    "status": "Статус стока",
    "ends": "Сток закончится",
    "code": "Код товара",
    "fact": "Факт прошл. недели",
    "plan": "План прошл. недели",
}
...
def filter_tags(rows, query):
    ...
    for row in rows:
        ...
        if (
            (query.tag_operator == "eq" and actual == value)
...
def economic_filter(rows, user, store, manager=None, article_key="article"):
    ...
    return [
        ...
        for r in rows
        if (
            str(r.get(article_key) or "") in allowed
...
def prices(query):
    if query.date_from:
        ...
        rows = select(
            db.get_unit_economics_1c_latest_daily_prices((query.store,)),
...
def stocks(query):
    ...
    detailed = read(
        "SELECT article, scheme, warehouse, quantity, updated_at FROM mp_warehouse_stock "
        "WHERE store_slug=? AND marketplace=?",
        (query.store, query.marketplace),
    )
    if query.warehouse:
...
def stock_summary(query):
    ...
    if query.fulfillment:
        selected = {
            s: db.get_mp_stock_by_warehouse(query.store, query.marketplace, s, query.fulfillment)
            for s, _ in schemes
            if s == "fbs" or s.startswith("fbs_")
        }
    for row in rows:
...
def operations(query, allowed_pairs):
    ...
    for row in rows:
        ...
        if query.kind and row.get("kind") != query.kind:
            continue
        safe = select(
            [row], "id kind created_at from_fulfillment to_fulfillment from_marketplace to_marketplace"
        )[0]
...
def filtered(rows, query):
    ...
    if query.search:
        ...
        result = [
            ...
            for r in result
            if any(
                term in str(r.get(k) or "").casefold()
...
def page(rows, query, metrics=(), *, sort_fields=()):
    ...
    if query.sort_by and key not in set(metrics) | set(sort_fields) | {
        "article",
        "day",
        "name",
        "updated_at",
    ...
    known.sort(
        key=lambda r: (r[key], str(r.get("article") or r.get("day") or "")), reverse=query.order == "desc"
    )
    rows = known + unknown
    totals = {
        key: sum(r[key] for r in rows) if rows and all(r.get(key) is not None for r in rows) else None
        for key in metrics
    }
    return {
...
def status(store, marketplace, section):
    sources = {
        ...
        "unit_economics_1c": [
            ("unit_economics_1c_wb_daily_prices", "updated_at", "day", True),
    ...
    for table, stamp, day, has_marketplace in sources.get(section, []):
        ...
        row = read(
            f"SELECT COUNT(*) AS records, MAX({stamp}) AS updated_at {period} "
            f"FROM {table} WHERE store_slug=? {clause}",
            values,
        )[0]
