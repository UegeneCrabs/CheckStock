...
from app import background, ftp_export, ftp_export_schedule, sync_locks, sync_settings
from app.repositories import core, yandex_assortment
from app.sync_tracking import set_next_run
...
def _storefront_now() -> dict:
    ...
    selected = sorted((slug, article) for slug, article in yandex_assortment.load_active_products() if slug in stores)
    if not selected:
    ...
    with collector.profile_lock(args.state_dir), sync_playwright() as playwright:
        context = playwright.chromium.launch_persistent_context(
            str(args.state_dir / "browser-profile"), headless=True,
...
def callback_for(name: str) -> Callable[[], object]:
    ...
    overrides = {
        "catalog_sync": lambda: _marketplaces_now("catalog_sync", (
            ("WB", background.wb_catalog.sync_all), ("OZON", background.ozon_catalog.sync_all),
            ("YANDEX MARKET", background.ya_catalog.sync_all))),
        "stock_sync": lambda: _marketplaces_now("stock_sync", (
            ("WB", background.wb_sync.sync_all), ("OZON", background.ozon_sync.sync_all),
