...
def weekly_metrics(
    ...
    if advertising_per_unit is None:
        if not available:
            advertising_warnings.append(
                "Нет совместных данных заказов и рекламы для расчёта рекламы на штуку."
            )
        elif count <= 0:
            advertising_warnings.append(
                "Есть расходы на рекламу, но нет заказов: рекламу на штуку рассчитать нельзя."
            )
        else:
...
def calculate_row(
    *,
    ...
    target_drr = product_target_drr if product_target_drr is not None else cabinet.target_drr_percent
    target_roi = product_target_roi if product_target_roi is not None else code_target_roi
    warnings = []
