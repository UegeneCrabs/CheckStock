...
def _build_reconciliation(
    ...
    for operation in operations:
        is_legacy_fbs_transfer = operation.get("kind") == "shipment" and operation.get("is_fbs_transfer")
...
def build_report(
    ...
    operations = [
        ...
        if (
            ...
            and (allowed is None or (operation["store_slug"], operation.get("from_marketplace")) in allowed)
        )
        or (
            operation.get("to_marketplace") in marketplaces
            and (allowed is None or (operation["store_slug"], operation.get("to_marketplace")) in allowed)
        )
    ]
    ...
    if allowed is not None:
        summary = [row for row in summary if (row["store_slug"], row["marketplace"]) in allowed]
    ...
    if allowed is not None:
        reconciliation = [row for row in reconciliation if (row["store_slug"], row["marketplace"]) in allowed]
