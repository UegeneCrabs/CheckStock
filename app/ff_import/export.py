...
def build_operation_xlsx(operation_id: int) -> tuple[bytes, str]:
    ...
    name = _safe_filename(f"{operation['kind']}_{operation['store_slug']}_{date_part}_{operation_id}")
...
def build_history_xlsx(store_slug: str, store_name: str, operations: list[dict]) -> tuple[bytes, str]:
...
def build_warehouses_xlsx(
    store_slug: str, store_name: str, marketplace: str, tables: list[tuple[str, list[dict]]]
) -> tuple[bytes, str]:
