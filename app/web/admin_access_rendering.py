...
def render_section_fields(user: User, editable: bool) -> str:
    ...
    for title, sections in SECTION_GROUPS:
        ...
        for section in sections:
            ...
            defaults = user.model_copy(update={"section_access": {key: value for key, value in user.section_access.items() if key is not section}})
            default_label = LEVEL_LABELS[access_level(defaults, section)]
            options = [f'<option value="" title="{default_label}"{" selected" if configured is None else ""}>По умолчанию</option>']
            for value, label in LEVEL_LABELS.items():
                allowed = value is SectionAccessLevel.NONE or value is limit or (value is SectionAccessLevel.READ and limit is SectionAccessLevel.WRITE)
                if allowed or configured is value:
                    options.append(f'<option value="{value.value}"{" selected" if configured is value else ""}>{label}</option>')
            ...
            if section in SUPERADMIN_SECTIONS:
                ...
                control = '<span class="ad-permission-fixed">Недоступно для роли, должности или площадки</span>'
                if editable:
