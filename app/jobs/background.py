...
from app.economics.wb import history as unit_margin_history
...
def _sync_catalogs_configured() -> SyncGroupReport:
    return _run_sync_group(
        ...
        (
            ...
            (
                "YANDEX MARKET",
                lambda: ya_catalog.sync_all(sync_settings.enabled_stores("catalog_sync", "YANDEX MARKET")),
            ),
        ),
    )
...
def _sync_stocks_configured() -> SyncGroupReport:
    return _run_sync_group(
        ...
        (
            ...
            (
                "YANDEX MARKET",
                lambda: ya_sync.sync_all(sync_settings.enabled_stores("stock_sync", "YANDEX MARKET")),
            ),
        ),
    )
...
def _sync_prices_due_configured() -> dict[str, dict]:
    return unit_economics_1c.sync_prices_due(sync_settings.enabled_stores("unit_economics_1c_sync"))


def _sync_wallet_prices_configured() -> dict[str, dict]:
    return unit_economics_1c.sync_wallet_prices(sync_settings.enabled_stores("unit_economics_1c_wallet_sync"))


def _save_daily_margin_configured() -> dict:
    return unit_margin_history.save_daily_margin_snapshots(
        store_slugs=sync_settings.enabled_stores("unit_economics_1c_daily_margin_snapshot_00_msk")
    )
        ...
        jobs.append(
            BackgroundJob(
                ...
                is_enabled=lambda current_job=job_name: (
                    _job_enabled(current_job) and ftp_export_schedule.should_attempt(current_job)
                ),
            )
        )
    return tuple(jobs)
