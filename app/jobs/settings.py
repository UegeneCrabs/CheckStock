...
from app.jobs.catalog import SyncJobDefinition, job_definitions
...
def _setting_map(name: str) -> dict[tuple[str, str], bool]:
    ...
    return {
        (str(row.get("store_slug") or ""), str(row.get("marketplace") or "")): bool(row.get("enabled"))
        for row in rows
    }
...
def _targets(definition: SyncJobDefinition) -> tuple[tuple[str, str], ...]:
    ...
    if definition.scope == "store_marketplaces":
        return tuple(
            (store_slug, marketplace) for store_slug in STORES for marketplace in definition.marketplaces
        )
    return ()
...
def configuration(name: str) -> dict:
    ...
    marketplaces_enabled = {item["marketplace"]: item["enabled"] for item in marketplace_settings}
    targets = [
        {
...
def save_setting(
    ...
    *,
    ...
    if not store_slug and not marketplace:
        ...
        definition.scope == "store_marketplaces" and not store_slug and marketplace in definition.marketplaces
    ):
        pass
