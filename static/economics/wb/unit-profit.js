(function () {
    ...
    var state = {
        ...
        page: 1,
        pageSize:
            [25, 50, 100].indexOf(Number(preferences.pageSize)) !== -1 ? Number(preferences.pageSize) : 50,
        totalCount: 0,
        totalPages: 1,
    ...
    var reportColumnGroups = [
        {
            ...
            label: 'Товар',
            fixed: true,
            columns: [{ key: 'identity', label: 'Товар', format: 'identity' }],
        },
        {
            key: 'store',
            label: 'Магазин',
            columns: [{ key: 'store_name', label: 'Магазин', format: 'text' }],
        },
        {
            key: 'subject',
            label: 'Категория',
            columns: [{ key: 'subject', label: 'Категория', format: 'text' }],
        },
        {
            key: 'manager',
            label: 'Менеджер',
            columns: [{ key: 'manager', label: 'Менеджер', format: 'text' }],
        },
        {
            key: 'funnel',
            ...
            columns: [
                {
                    ...
                    label: 'Всего',
                    format: 'number',
                    help: 'Текущие остатки WB: FBS + FBO + ФФ',
                },
        ...
        },
        {
            ...
            columns: [
                ...
                { key: 'advertising_spend', label: 'Расходы, ₽', format: 'money' },
                {
                    ...
                    label: 'ДРР',
                    format: 'percent',
                    help: 'Доля рекламных расходов с учётом выкупа',
                },
            ],
        },
        {
    ...
    if (config.marketplace === 'YM') {
        ...
        dailyColumns = [
            ...
            { key: 'logistics_delivery', label: 'Доставка выкупа, ₽', format: 'money' },
            { key: 'logistics_returns', label: 'Обратная доставка невыкупов, ₽', format: 'money' },
            { key: 'logistics_repeat_delivery', label: 'Повторная доставка, ₽', format: 'money' },
            { key: 'logistics_transit', label: 'Транзит, ₽', format: 'money' },
            { key: 'logistics_adjustment', label: 'Корректировка логистики, ₽', format: 'money' },
            { key: 'logistics', label: 'Логистика итого, ₽', format: 'money' },
            { key: 'loss_percent', label: 'Потери от закупки, %', format: 'percent' },
            { key: 'loss', label: 'Потери от закупки, ₽', format: 'money' },
    ...
    function rub(item) {
        return item === null || item === undefined ? '—' : money.format(item);
    }
    ...
    function copyIdentifier(kind, item, label) {
        ...
        return window.CheckStockUI.render('economics/wb/unit-profit/copy-identifier', {
            kind: kind,
            item: item,
            content: kind.toLocaleLowerCase('ru-RU'),
            item_2: item,
            label: label,
        });
    }
    ...
    function summary(kind) {
        ...
        summaryNodes[kind].textContent = count
            ? count + ' ' + (count < 5 ? labels[1] : labels[2])
            : labels[0];
    }
    function optionHtml(kind, item) {
        ...
        if (kind === 'article') {
            label = window.CheckStockUI.render('economics/wb/unit-profit/option-html', {
                name: item.name,
                article: item.article,
                store_name: item.store_name,
            });
        } else
            label = window.CheckStockUI.render('economics/wb/unit-profit/option-html-2', {
                label: label,
            });
        return window.CheckStockUI.render('economics/wb/unit-profit/option-html-3', {
            kind: kind,
            rawValue: rawValue,
    ...
    function renderOptions(kind, items) {
        ...
        optionNodes[kind].innerHTML = items.length
            ? items
                  .map(function (item) {
                      return optionHtml(kind, item);
                  })
                  .join('')
            : window.CheckStockUI.render('economics/wb/unit-profit/render-options');
        summary(kind);
    }
    function renderFilterOptions(kind) {
        var query = searchNodes[kind] ? searchNodes[kind].value.trim().toLocaleLowerCase('ru-RU') : '';
        var filtered = filterOptions[kind].filter(function (item) {
            var text =
                kind === 'article'
                    ? [item.article, item.name, item.store_name].join(' ')
                    : kind === 'store'
                      ? [item.slug, item.name].join(' ')
                      : String(item);
    ...
    function productMedia(row) {
        ...
        return window.CheckStockUI.render('common/ui/lazy-product-image', {
    ...
    function reportGroupByKey(key) {
        return reportColumnGroups.find(function (group) {
    ...
    function visibleColumnGroups() {
        return columnPreferences.order
            .map(reportGroupByKey)
            .filter(Boolean)
            .map(function (group) {
                return {
                    key: group.key,
    ...
    function reportColumnLabel(column) {
        var label = state.viewMode === 'categories' && column.key === 'identity' ? 'Категория' : column.label;
        var content = escapeHtml(label);
        if (column.help) {
            content = window.CheckStockUI.render('economics/wb/unit-profit/report-column-label', {
                help: column.help,
                content: content,
            });
        }
    ...
    function headerTotalHtml(column) {
        if (column.key === 'identity') {
            return window.CheckStockUI.render('economics/wb/unit-profit/header-total-html');
        }
        if (!column.key || ['store_name', 'subject', 'manager'].indexOf(column.key) !== -1) {
            return window.CheckStockUI.render('economics/wb/unit-profit/header-total-html-2');
        }
        return window.CheckStockUI.render('economics/wb/unit-profit/header-total-html-3', {
            key: column.key,
        });
    }
    function reportColumnHeader(column, columnIndex, groupKey) {
        var numeric = ['number', 'turnover', 'money', 'percent'].indexOf(column.format) !== -1;
        return window.CheckStockUI.render('economics/wb/unit-profit/report-column-header', {
            content: numeric ? 'num' : '',
            groupKey: groupKey,
            columnIndex: columnIndex,
            content_2: numeric ? ' data-filter-type="number"' : '',
            column: reportColumnLabel(column),
            column_2: headerTotalHtml(column),
        });
    }
    function renderBaseHeaders() {
        ...
        groups.forEach(function (group) {
            if (group.columns.length === 1) {
                groupHtml += reportColumnHeader(group.columns[0], columnIndex, group.key).replace(
                    window.CheckStockUI.render('economics/wb/unit-profit/render-base-headers'),
                    window.CheckStockUI.render('economics/wb/unit-profit/render-base-headers-2'),
                );
                columnIndex += 1;
                return;
            }
            groupHtml += window.CheckStockUI.render('economics/wb/unit-profit/render-base-headers-3', {
        ...
        nodes.tableHead.innerHTML = window.CheckStockUI.render(
            'economics/wb/unit-profit/render-base-headers-4',
            { groupHtml: groupHtml, metricHtml: metricHtml },
        );
    ...
    function renderDailyHeaders() {
        ...
        state.dailyDates.forEach(function (day, dayIndex) {
            ...
            dailyColumns.forEach(function (column) {
                ...
                heading.className =
                    'num ue1cr-daily-column ue1cr-daily-metric ue1cr-daily-day-' + (dayIndex % 2);
                heading.scope = 'col';
    ...
    function identityHtml(row) {
        if (row.row_kind === 'category') {
            return window.CheckStockUI.render('economics/wb/unit-profit/identity-html', {
                name: row.name,
                product_count: value(row.product_count),
            });
        }
        return window.CheckStockUI.render('economics/wb/unit-profit/identity-html-2', {
            row: productMedia(row),
            name: row.name,
            content: copyIdentifier('Артикул', row.article, 'Арт. ' + row.article),
        });
    }
    function reportCellHtml(row, column) {
        if (column.key === 'identity')
            return window.CheckStockUI.render('economics/wb/unit-profit/report-cell-html', {
                row: identityHtml(row),
            });
        ...
        var filterValue =
            Number.isFinite(parsed) && column.format !== 'text'
                ? ' data-filter-value="' + String(parsed) + '"'
                : '';
        return window.CheckStockUI.render('economics/wb/unit-profit/report-cell-html-2', {
            classes: classes,
            ...
            .map(function (group) {
                return group.columns
                    .map(function (column) {
                        ...
                        return cell.replace(
                            window.CheckStockUI.render('economics/wb/unit-profit/report-cells-html'),
                            window.CheckStockUI.render('economics/wb/unit-profit/report-cells-html-2', {
                                key: group.key,
                            }),
                        );
                    })
    ...
    function rowHtml(row, index) {
        return window.CheckStockUI.render('economics/wb/unit-profit/row-html', {
            index: index,
            content: row.row_kind || 'product',
            row: reportCellsHtml(row),
            row_2: dailyCellsHtml(row),
        });
    }
    ...
    function renderHeaderTotals(total, rowCount) {
        ...
        if (countNode) {
            countNode.textContent =
                'Итого: ' + value(rowCount) + (state.viewMode === 'categories' ? ' кат.' : ' поз.');
        }
    ...
    function aggregateRows(rows) {
        var total = rows.reduce(
            ...
            },
            {
                ...
                stock_fbo: 0,
                stock_fulfillment: 0,
                stock_average_daily_orders: 0,
                impressions: 0,
                clicks: 0,
                advertising_spend: 0,
                expected_buyout_amount: 0,
                margin_orders_count: 0,
                margin: 0,
                purchase_value: 0,
        ...
        total.stock_days =
            total.stock_average_daily_orders > 0
                ? Math.round((total.stock / total.stock_average_daily_orders) * 100) / 100
                : 0;
        total.buyout_percent = total.buyout_orders_count
            ? Math.round((total.buyout_weighted / total.buyout_orders_count) * 100) / 100
            : null;
        total.ctr = total.impressions ? Math.round((total.clicks / total.impressions) * 10000) / 100 : 0;
        total.cpc = total.clicks ? Math.round((total.advertising_spend / total.clicks) * 100) / 100 : 0;
        total.drr = total.expected_buyout_amount
            ? Math.round((total.advertising_spend / total.expected_buyout_amount) * 10000) / 100
            : total.advertising_spend
              ? 100
              : 0;
    ...
    function visibleReportRows() {
        return Array.prototype.map
            .call(nodes.rows.querySelectorAll('tr[data-row-index]'), function (row) {
    ...
    function updateDailyVisibility() {
        ...
        root.classList.toggle('ue1cr-page--daily-hidden', !state.showDailyDetails || categories);
    ...
    function renderRows() {
        ...
        renderHeaderTotals(state.totalCount ? state.totals : aggregateRows([]), state.totalCount);
    ...
    function renderColumnSettings() {
        nodes.columnList.innerHTML = columnPreferences.order
            .map(function (key, index) {
                ...
                var metricOptions =
                    !group.fixed && group.columns.length > 1
                        ? window.CheckStockUI.render('economics/wb/unit-profit/metric-options-2', {
                              content: group.columns
                                  .map(function (column) {
                                      var metricChecked =
                                          columnPreferences.hiddenColumns.indexOf(column.key) === -1;
                                      return window.CheckStockUI.render(
                                          'economics/wb/unit-profit/metric-options',
                                          {
                                              key: column.key,
                                              content: metricChecked ? ' checked' : '',
                                              label: column.label,
                                          },
                                      );
                                  })
                        ...
                        : '';
                return window.CheckStockUI.render('economics/wb/unit-profit/render-column-settings', {
                    key: key,
                    content: group.fixed ? 'false' : 'true',
                    content_2: checked ? ' checked' : '',
                    content_3: group.fixed ? ' disabled' : '',
                    label: group.label,
                    content_4: canMoveUp ? '' : ' disabled',
                    content_5: canMoveDown ? '' : ' disabled',
    ...
    function appendFilters(query, kind, parameter) {
        state[kind].forEach(function (item) {
            query.append(parameter, item);
        });
    }
    ...
    async function load(options) {
        ...
        try {
            ...
            state.dailyDates =
                state.dailyDetailsLoaded && state.rows.length
                    ? (state.rows[0].daily_calculations || []).map(function (item) {
                          return item.date;
                      })
                    : [];
            ...
            if (result.manager_scope && result.manager_scope.restricted && !result.manager_scope.matched) {
                nodes.scopeNote.textContent =
                    'В данных 1С не найден менеджер, соответствующий вашему пользователю.';
    ...
    nodes.viewButtons.forEach(function (button) {
        button.addEventListener('click', function () {
            setView(button.dataset.reportView);
        });
    });
    ...
    document.addEventListener('click', function (event) {
        ...
        if (
            nodes.columnPanel.hidden ||
            nodes.columnPanel.contains(event.target) ||
            nodes.columnsToggle.contains(event.target)
        )
            return;
    ...
    nodes.columnList.addEventListener('dragover', function (event) {
        ...
        if (
            !option ||
            !draggedColumnKey ||
            option.dataset.columnKey === 'product' ||
            option.dataset.columnKey === draggedColumnKey
        )
            return;
        event.preventDefault();
    ...
    window.addEventListener('focus', function () {
        refreshReportIfStale(false);
    });
    window.setInterval(function () {
        refreshReportIfStale(true);
