(function () {
    ...
    function node(id) {
        return document.getElementById('uetp-' + id);
    }
    var escape = window.CheckStockUI.escapeHtml;
    ...
    function copyIdentifier(article) {
        return window.CheckStockUI.render('economics/wb/target-price/copy-identifier', {
            article: article,
            article_2: article,
            article_3: article,
        });
    ...
    table._tfAdapter = {
        values: function (index) {
            return externallyFilteredRows().map(function (row) {
                return columnValue(row, index);
            });
        },
        ...
        sort: function (index, direction) {
            applySort(columns[Number(index)], direction);
        },
    };
    var help = node('help');
    ...
    function below(row) {
        return row.current_price != null && row.target_price != null && row.current_price < row.target_price;
    }
    function cell(value, warnings, extra, explanation, notes) {
        ...
        return window.CheckStockUI.render('economics/wb/target-price/cell', {
            content: numeric(value) === null ? '—' : String(Number(value)),
            content_2: warningText ? 'is-warning ' : '',
            content_3: extra || '',
            content_4: title
                ? ' tabindex="0" title="' +
                  escape(title) +
                  '" aria-label="' +
                  escape(format(value) + '. ' + title) +
                  '"'
                : '',
            value: format(value),
        });
    ...
    function filteredRows() {
        ...
        var visible = externallyFilteredRows().filter(function (row) {
            return activeColumns.every(function (index) {
                return tableFilters[index].has(columnValue(row, index));
            });
        });
        visible.sort(function (a, b) {
            ...
            if (index >= 0 && index < 3)
                return (
                    columnValue(a, index).localeCompare(columnValue(b, index), 'ru') * (descending ? -1 : 1)
                );
            var left = numeric(a[sort]),
                right = numeric(b[sort]);
    ...
    function render() {
        ...
        node('rows').innerHTML =
            visible
                ...
                .map(function (row) {
                    var image = /^https?:\/\//i.test(row.image_url)
                        ? window.CheckStockUI.render('common/ui/lazy-product-image', {
                              image_url: row.image_url,
                          })
                        : '';
                    var detail =
                        'Период: ' +
                        row.weekly.period_from +
                        ' — ' +
                        row.weekly.period_to +
                        '. Данные за ' +
                        row.weekly.days +
                        ' из 7 дней. Реклама: ' +
                        format(row.weekly.spend) +
                        ' ₽; заказы: ' +
                        format(row.weekly.orders_amount) +
                        ' ₽ (' +
                        format(row.weekly.orders_count) +
                        ' шт.); выкуп: ' +
                        format(row.weekly.buyout_percent) +
                        '%.';
                    var drrDetail =
                        detail +
                        (row.current_drr === 0
                    ...
                    var roiDetail =
                        detail +
                        (row.current_roi == null
                            ? '\nROI не рассчитан: см. причины ниже.'
                            : '\nROI рассчитан по текущей цене и параметрам калькулятора.') +
                        ' Реклама на выкупленную штуку: ' +
                        format(row.weekly.advertising_per_unit) +
                        ' ₽.';
                    var targetDetail =
                        row.target_retail_price == null
                            ? ''
                            : 'До СПП: ' +
                              format(row.target_retail_price) +
                              ' ₽; с СПП: ' +
                              format(row.target_spp_price) +
                              ' ₽. ROI после округления: ' +
                              format(row.target_actual_roi) +
                              '%.';
                    return window.CheckStockUI.render('economics/wb/target-price/render', {
                        content: columnValue(row, 0),
                        image: image,
                        content_2: row.store_slug + '|' + row.article,
                        name: row.name,
                        article: copyIdentifier(row.article),
                        store_name: row.store_name,
                        content_3: columnValue(row, 2),
                        content_4: cell(
                            row.current_price,
                            [],
                            below(row) ? 'is-below' : '',
                        ...
                        content_5: cell(
                            row.current_drr,
                            row.current_drr_warnings,
                            '',
                            drrDetail,
                            row.current_drr_notes,
                        ),
                        content_6: cell(
                            row.current_roi,
                            row.current_warnings,
                            row.current_roi < 0 ? 'is-negative' : '',
                            roiDetail,
                            row.current_notes,
                        ),
                        content_7: cell(
                        ...
                        content_8: cell(
                            row.target_drr,
                            [],
                            row.target_overridden ? 'uetp-target-custom' : '',
                            row.target_overridden
                                ? 'Индивидуальная цель товара'
                                : 'Цель из настроек кабинета ' + row.store_name,
                        ),
                        content_9: cell(
                            row.target_roi,
                            [],
                ...
                .join('') || window.CheckStockUI.render('economics/wb/target-price/render-2');
        node('count').textContent =
            'Товаров: ' + visible.length + ' · Цена ниже целевой: ' + visible.filter(below).length;
        node('page').textContent = page + ' / ' + pages;
        node('prev').disabled = page <= 1;
        node('next').disabled = page >= pages;
    ...
    async function load() {
        var id = ++requestId;
        node('reload').disabled = true;
        status('Загрузка отчёта…');
        rows = [];
        page = 1;
        render();
        try {
            ...
            if (id === requestId) status(error.message || 'Ошибка загрузки', true);
        } finally {
            if (id === requestId) node('reload').disabled = false;
        }
        return false;
    }
    node('reload').addEventListener('click', load);
    node('store').addEventListener('change', load);
    ...
    node('prev').addEventListener('click', function () {
        page -= 1;
        render();
    });
    node('next').addEventListener('click', function () {
        page += 1;
        render();
    });
    ...
    table.addEventListener('click', function (event) {
        ...
        if (opener) {
            ...
            var row = rows.find(function (item) {
                return item.store_slug + '|' + item.article === key;
            });
    ...
    function syncPrices(source) {
        var retail = calcValue('retail'),
            client = calcValue('client'),
            wallet = calcValue('wallet');
        var spp = calcValue('spp'),
            walletPercent = calcValue('wallet_percent');
        if ((source === 'retail' || source === 'spp') && retail !== null && spp !== null) {
            client = Math.round(retail * (1 - spp / 100));
            setCalc('client', client);
            if (walletPercent !== null) setCalc('wallet', client - Math.ceil((client * walletPercent) / 100));
        } else if (source === 'client' && client !== null && spp !== null) {
            retail = client / Math.max(0.0001, 1 - spp / 100);
            setCalc('retail', retail);
            if (walletPercent !== null) setCalc('wallet', client - Math.ceil((client * walletPercent) / 100));
            ...
            client = Math.round(wallet / Math.max(0.0001, 1 - walletPercent / 100));
            setCalc('client', client);
            setCalc('retail', client / Math.max(0.0001, 1 - spp / 100));
        }
    }
    ...
    function syncPercentAndRub(percentKey, rubKey, source, base, priceChanged) {
        var percent = calcValue(percentKey),
            rubles = calcValue(rubKey);
        if (source === rubKey) {
    ...
    function syncCalculatedAmounts(source) {
        ...
        var storageRate = calcValue('storage_wb_rub'),
            turnoverDays = calcValue('turnover_days');
        var storageTotal = calcValue('storage_total');
        if (source === 'storage_total') {
            setCalc(
                'storage_wb_rub',
                turnoverDays === null || turnoverDays <= 0 || storageTotal === null
                    ? null
                    : storageTotal / turnoverDays,
            );
        ...
        var vatPercent = calcValue('vat_percent'),
            vatRub = calcValue('vat_rub');
        if (source === 'vat_rub') {
            setCalc(
                'vat_percent',
                client === null || vatRub === null || vatRub >= client
                    ? null
                    : (vatRub * 100) / (client - vatRub),
            );
        ...
        var secondaryBase =
            calculatorTaxSystem() === 'osno'
                ? client
                : client === null || vatRub === null
                  ? null
                  : client - vatRub;
        if (source === 'secondary_tax_rub') {
            setCalc(
                'secondary_tax_percent',
                secondaryBase === null || secondaryBase <= 0 || secondaryRub === null
                    ? null
                    : (secondaryRub / secondaryBase) * 100,
            );
        } else if (
            source === 'secondary_tax_percent' ||
            source === 'vat_percent' ||
            source === 'vat_rub' ||
            source === 'fill' ||
            priceChanged
            ...
            setCalc(
                'secondary_tax_rub',
                secondaryBase === null || secondaryPercent === null
                    ? null
                    : (secondaryBase * secondaryPercent) / 100,
            );
        }
    }
    function calculate() {
        if (!calculatorRow) return;
        var targetPrice = numeric(calculatorRow.target_price),
            walletPrice = calcValue('wallet');
        ...
        var targetTile = window.CheckStockUI.render('economics/wb/target-price/target-tile', {
            targetPrice: format(targetPrice),
            content: targetPrice === null ? '' : ' ₽',
        });
        var retail = calcValue('retail'),
            client = calcValue('client'),
            purchase = calcValue('purchase_price');
        var required = [
            'acquiring_rub',
            'delivery_with_returns',
            'storage_total',
        ...
        if (
            ...
            required.some(function (key) {
                return calcValue(key) === null;
            })
        ) {
            node('calculator-results').innerHTML = window.CheckStockUI.render(
                'economics/wb/target-price/calculate',
                { targetTile: targetTile },
            );
            return;
        }
        var vat = calcValue('vat_rub'),
            secondary = calcValue('secondary_tax_rub');
        var acquiring = calcValue('acquiring_rub'),
            commission = calcValue('wb_commission_rub');
        var team = calcValue('team_commission_rub'),
            storage = calcValue('storage_total');
        var net =
            retail -
            acquiring -
            calcValue('delivery_with_returns') -
            storage -
            commission -
            calcValue('advertising_rub');
        var margin = net - purchase - calcValue('fulfillment_cost') - team - vat - secondary;
        var roi = purchase > 0 ? (margin / purchase) * 100 : null;
        node('calculator-results').innerHTML = window.CheckStockUI.render(
            'economics/wb/target-price/calculate-2',
            {
                content: margin < 0 ? 'is-negative' : 'is-positive',
                margin: format(margin),
                content_2: roi !== null && roi < 0 ? 'is-negative' : 'is-positive',
                roi: format(roi),
                targetTile: targetTile,
            },
        );
    ...
    function solveTargetPrice() {
        ...
        var spp = calcValue('spp'),
            targetRoi = calcValue('target_roi'),
            purchase = calcValue('purchase_price');
        var drr = calcValue('drr'),
            buyoutPercent = calcValue('buyout_percent');
        var required = [
            'acquiring_percent',
            'delivery_with_returns',
            'storage_wb_rub',
        ...
        if (
            ...
            return;
        var customerFactor = Math.max(0.0001, 1 - spp / 100);
        ...
        var revenueFactor =
            1 -
            (calcValue('acquiring_percent') +
                calcValue('wb_commission_percent') +
                calcValue('team_commission_percent')) /
                100 -
            vatFactor -
            secondaryFactor;
        ...
        var requiredRetail =
            (purchase +
                (purchase * targetRoi) / 100 +
                calcValue('fulfillment_cost') +
                calcValue('delivery_with_returns') +
                calcValue('storage_wb_rub') * calcValue('turnover_days')) /
            revenueFactor;
        ...
        var best = null;
        for (
            var retailCents = Math.max(centerCents - 150, 1);
            retailCents <= centerCents + 150;
            retailCents += 1
        ) {
            ...
            if (
                ...
                best = {
                    ...
                    retail: retailCandidate,
                    advertising: advertisingCandidate,
                    score: score,
                    distance: Math.abs(retailCents - requiredRetail * 100),
                };
            }
        }
        if (!best) return;
        var client = best.client,
            retail = best.retail;
        setCalc('retail', retail);
        setCalc('client', client);
    ...
    function fillCalculator(values) {
        root.querySelectorAll('[data-calc]').forEach(function (input) {
            input.value = '';
        });
        Object.keys(values || {}).forEach(function (key) {
            if (root.querySelector('[data-calc="' + key + '"]')) setCalc(key, values[key]);
        });
        setCalc(
            'secondary_tax_percent',
            values.tax_system === 'osno' ? values.osno_percent : values.usn_percent,
        );
    ...
    function openCalculator(row) {
        calculatorRow = row;
        calculatorInitial = Object.assign({}, row.calculator || {});
        var initial = String(row.name || row.article || '?')
            .slice(0, 1)
            .toLocaleUpperCase('ru-RU');
        node('drawer-thumb').innerHTML = window.CheckStockUI.render('common/ui/product-media', {
            initial: initial,
            content: /^https?:\/\//i.test(row.image_url || '')
        ...
        targetStatus('', false);
        root.classList.add('has-calculator');
        document.body.classList.add('uetp-calculator-open');
        node('drawer').classList.add('is-open');
        node('overlay').classList.add('is-open');
        node('drawer').setAttribute('aria-hidden', 'false');
        fillCalculator(calculatorInitial);
    }
    function closeCalculator() {
        calculatorRow = null;
        root.classList.remove('has-calculator');
        document.body.classList.remove('uetp-calculator-open');
        node('drawer').classList.remove('is-open');
        node('overlay').classList.remove('is-open');
        node('drawer').setAttribute('aria-hidden', 'true');
    }
    root.querySelectorAll('[data-calc]').forEach(function (input) {
        input.addEventListener('input', function () {
            var source = input.dataset.calc;
            syncPrices(source);
            var priceChanged = ['retail', 'spp', 'client', 'wallet_percent', 'wallet'].indexOf(source) !== -1;
            ...
            if (source === 'drr' || source === 'buyout_percent' || priceChanged) {
                setCalc(
                    'advertising_rub',
                    calculatedAdvertisingRub(
                        calcValue('retail'),
                        calcValue('drr'),
                        calcValue('buyout_percent'),
                    ),
                );
            }
            ...
            else if (source === 'advertising_rub')
                setCalc(
                    'drr',
                    calculatedDrrPercent(
                        calcValue('retail'),
                        calcValue('advertising_rub'),
                        calcValue('buyout_percent'),
                    ),
    ...
    async function refreshOpenCalculator(message) {
        ...
        var updated = rows.find(function (item) {
            return item.store_slug + '|' + item.article === key;
        });
        if (!updated) {
            closeCalculator();
        ...
        openCalculator(updated);
        targetStatus(message, false);
        return true;
    }
    ...
    node('target-save').addEventListener('click', async function () {
        ...
        var drr = calcValue('drr'),
            roi = calcValue('target_roi');
        if (drr === null || drr < 0 || drr > 100 || roi === null || roi < 0 || roi > 1000000) {
            targetStatus('Проверьте целевые ДРР и ROI.', true);
            return;
        }
        var button = node('target-save');
        button.disabled = true;
        node('calculator-reset').disabled = true;
        try {
            var response = await fetch(
                '/api/unit-economics-1c/reports/target-price/' +
                    encodeURIComponent(calculatorRow.store_slug) +
                    '/targets',
                {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'fetch' },
                    body: JSON.stringify({
                        article: calculatorRow.article,
                        target_drr_percent: drr,
                        target_roi_percent: roi,
                    }),
                },
            );
            var data = await response.json();
            ...
            targetStatus(error.message || 'Не удалось сохранить цели', true);
        } finally {
    ...
    node('calculator-reset').addEventListener('click', async function () {
        if (!calculatorRow || !config.canEdit) return;
        var button = node('calculator-reset');
        button.disabled = true;
        node('target-save').disabled = true;
        try {
            var response = await fetch(
                '/api/unit-economics-1c/reports/target-price/' +
                    encodeURIComponent(calculatorRow.store_slug) +
            ...
            targetStatus(error.message || 'Не удалось сбросить цели', true);
        } finally {
            button.disabled = !config.canEdit;
            node('target-save').disabled = !config.canEdit;
        }
    });
    node('drawer-close').addEventListener('click', closeCalculator);
    node('overlay').addEventListener('click', closeCalculator);
    document.addEventListener('keydown', function (event) {
    ...
    function resizeTable() {
        ...
        wrap.style.maxHeight =
            Math.max(240, Math.floor(viewportHeight - top - node('footer').offsetHeight - 16)) + 'px';
    }
    var resizeFrame;
    function scheduleResize() {
        window.cancelAnimationFrame(resizeFrame);
        resizeFrame = window.requestAnimationFrame(resizeTable);
    }
    ...
    if (window.ResizeObserver) {
        ...
        [
            root.querySelector('.uetp-heading'),
            root.querySelector('.uetp-controls'),
            node('status'),
            node('footer'),
