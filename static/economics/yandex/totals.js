(function () {
    ...
    window.CheckStockYandexTotals = function (products) {
        ...
        var current = products.filter(function (p) {
            ...
            return known(c.margin) && known(c.orders) && known(c.buyout_percent) &&
                known(currentPurchase(p));
        });
        ...
        ratio(3, current, currentProfit, function (p) { return currentPurchase(p) * bought(p); },
