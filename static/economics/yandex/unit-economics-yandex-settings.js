(function () {
    ...
    (config.items || []).forEach(function (item) {
        ...
        head.appendChild(heading); form.appendChild(head);
        var body = document.createElement('div');
        body.className = 'ue1cs-card-body';
        var group = document.createElement('div');
        group.className = 'ue1cs-field-group ue1cs-fields';
        body.appendChild(group); form.appendChild(body);
        [
            ...
            title.textContent = field[1]; label.appendChild(title);
            var wrap = document.createElement('span');
            wrap.className = 'ue1cs-input-wrap';
            var input = document.createElement('input');
            input.type = 'number'; input.name = field[0]; input.min = field[2];
            input.max = field[3]; input.step = field[4];
            input.required = field[0] === 'buyout_period_days';
            input.value = item[field[0]] == null ? '' : item[field[0]];
            input.disabled = !config.canEdit;
            wrap.appendChild(input); label.appendChild(wrap); group.appendChild(label);
        });
        var foot = document.createElement('div');
        foot.className = 'ue1cs-card-foot';
        var button = document.createElement('button');
        button.type = 'submit';
        button.textContent = 'Сохранить'; button.disabled = !config.canEdit;
        foot.appendChild(button); form.appendChild(foot);
        form.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                var response = await fetch('/api/unit-economics-1c/yandex-market/buyout-settings/' + encodeURIComponent(item.store_slug), {
                    method: 'PUT', headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({buyout_period_days: Number(form.elements.buyout_period_days.value),
            ...
            finally { button.disabled = !config.canEdit; }
        });
