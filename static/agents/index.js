(() => {
        ...
        if (!response.ok)
            throw new Error(
                typeof data.detail === 'string'
                    ? data.detail
                    : data.error || 'Не удалось выполнить запрос. Проверьте данные и повторите.',
            );
        return data;
    }
    async function load() {
        ...
        for (const key of data.keys) {
            ...
            for (const value of [
                key.name,
                key.id,
                new Date(key.expires_at).toLocaleString('ru-RU'),
                key.active ? 'Активен' : 'Истёк',
            ]) {
    ...
    form.onsubmit = async (event) => {
        ...
        try {
            ...
            secret.hidden = false;
            token.focus();
            tell('Подключение создано. Сохраните ключ.');
            await load();
        } catch (error) {
            tell(error.message);
        } finally {
    ...
    async function copy(text) {
        try {
            ...
            /* Some browsers block Clipboard API, especially over HTTP. */
        }
        ...
        tell(
            'Автоматическое копирование недоступно. Текст выделен ниже — нажмите Ctrl+C или выберите «Копировать» в меню выделения.',
        );
