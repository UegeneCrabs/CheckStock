(() => {
    ...
    if (!root) return;
    const state = { tab: 'users', query: '', role: '', status: '', store: '', sort: 'name', page: 1, log: '', requests: '' };
    const pageSize = 15;
    ...
    function toast(message, error = false) {
        ...
        box.hidden = false;
        toastTimer = setTimeout(() => { box.hidden = true; }, error ? 10000 : 4500);
    }
    ...
    async function post(url, body = new FormData()) {
        ...
        catch (_) { throw new Error('Сервер не вернул результат. Обновите страницу, чтобы проверить состояние.'); }
        if (!response.ok || !result.ok) {
            const error = new Error(result.error || 'Не удалось выполнить действие. Проверьте соединение и попробуйте снова.');
    ...
    function selectTab(tab) {
        ...
        all('[data-pane]').forEach(pane => { pane.hidden = pane.dataset.pane !== state.tab; });
    }
    ...
    function filterUsers() {
        ...
        const filtered = rows.filter(row => {
            row.hidden = true;
            return terms.every(term => normalize(row.dataset.search).includes(term))
        ...
        one('#ad-page-label').textContent = filtered.length
            ? `${start + 1}–${Math.min(start + pageSize, filtered.length)} из ${filtered.length} · Страница ${state.page} из ${pages}` : 'Нет записей';
    ...
    function syncFilters() {
        for (const [key, id] of Object.entries({ query: 'ad-user-search', role: 'ad-filter-role', status: 'ad-filter-status', store: 'ad-filter-store', sort: 'ad-sort' })) {
    ...
    function filterTable(key) {
        ...
        one(`[data-table-result="${key}"]`).textContent = rows.length
            ? (count ? `Показано записей: ${count} из ${rows.length}` : 'По вашему запросу ничего не найдено.') : '';
    }
    ...
    function confirmation(options) {
        ...
        return new Promise(resolve => {
            ...
            dialog.oncancel = event => { event.preventDefault(); if (options.cancel !== false) finish(false); };
    ...
    async function requestClose(dialog) {
        ...
        if (all('form', dialog).some(dirty)) {
            const discard = await confirmation({ title: 'Закрыть без сохранения?', text: 'В форме есть несохранённые изменения. Они будут потеряны.', label: 'Не сохранять', danger: true });
            if (!discard) return;
        }
        dialog.close();
    ...
    function updatePolicy(form) {
        ...
        if (storeButton) storeButton.textContent = all('[name="stores"]', form).every(input => input.checked) ? 'Снять выбор' : 'Выбрать все';
        ...
        if (preview) {
            const stores = all('[name="stores"]:checked', form).map(input => input.closest('label').textContent.trim());
            const markets = all('[name="marketplaces"]:checked', form).map(input => input.value);
            preview.textContent = stores.length && markets.length
                ? `Рабочая область: ${stores.join(', ')}. В каждом кабинете: ${markets.join(', ')}.`
                : 'Выберите хотя бы один кабинет и одну площадку.';
        }
        const single = ['marketplace_manager', 'senior_marketplace_manager', 'marketplace_lead'].includes(profile.value);
        one('[data-policy-hint]', form).textContent = single ? 'Для этой должности выберите ровно один маркетплейс.' : 'Можно выбрать несколько маркетплейсов.';
    }
    ...
    function editorTab(tab) {
        ...
        all('[data-editor-pane]').forEach(pane => { pane.hidden = pane.dataset.editorPane !== tab; });
    }
    ...
    function validatePolicy(form) {
        ...
        if (!count) throw Object.assign(new Error('Выберите хотя бы один маркетплейс.'), { field: 'marketplaces' });
        if (['marketplace_manager', 'senior_marketplace_manager', 'marketplace_lead'].includes(profile) && count !== 1) {
            throw Object.assign(new Error('Для этой должности выберите ровно один маркетплейс.'), { field: 'marketplaces' });
        }
    }

    async function refresh(options = {}) {
        const response = await fetch('/admin', { headers: { 'X-Requested-With': 'fetch' }, cache: 'no-store' });
        ...
        if (options.userId && one(`#ad-user-${options.userId}`)) {
            ...
            for (const draft of drafts) {
                ...
                all('[name]', form).forEach(input => {
                    const values = draft.entries.filter(entry => entry[0] === input.name).map(entry => entry[1]);
    ...
    function otherDrafts(form) {
        return all('.ad-edit-form').filter(item => item !== form && dirty(item))
            .map(item => ({ endpoint: item.dataset.endpoint, entries: Array.from(new FormData(item).entries()) }));
    }
    ...
    async function saveEditor(form) {
        ...
        try {
            ...
            if (['role', 'access-policy'].includes(form.dataset.endpoint) && drafts.length) {
                if (!await confirmation({ title: 'Изменить модель доступа?', text: 'В других блоках есть несохранённые изменения. Совместимые настройки останутся в форме. Настройки, недоступные для новой роли или должности, будут сброшены.', label: 'Продолжить' })) return;
            }
            ...
            all('.ad-edit-form button[type="submit"]').forEach(button => { button.disabled = true; });
            ...
            status.textContent = saved ? 'Сохранено. Обновите страницу, чтобы увидеть результат.' : error.message;
            status.classList.add('is-error');
        } finally {
            pending = false;
    ...
    function generatePassword() {
        ...
        return Array.from(crypto.getRandomValues(new Uint32Array(16)), value => alphabet[value % alphabet.length]).join('');
    }

    async function copyPassword(value, status) {
        if (!value) { status.textContent = 'Сначала введите или сгенерируйте пароль.'; return; }
        try { await navigator.clipboard.writeText(value); status.textContent = 'Пароль скопирован'; }
        catch (_) { status.textContent = 'Не удалось скопировать. Выделите пароль и скопируйте вручную.'; }
    }
    ...
    async function userAction(button) {
        ...
        const descriptions = {
            delete: ['Удалить сотрудника?', `${name}\nСотрудник потеряет доступ к системе. Учётная запись будет удалена, записи в журнале останутся.`, 'Удалить'],
            'toggle-active': [blocked ? 'Разблокировать сотрудника?' : 'Заблокировать сотрудника?', `${name}\n${blocked ? 'Сотрудник снова сможет войти в систему.' : 'Текущие сеансы завершатся. Войти снова можно будет после разблокировки.'}`, blocked ? 'Разблокировать' : 'Заблокировать'],
            'reset-password': ['Сбросить пароль?', `${name}\nВсе сеансы сотрудника завершатся. После сброса передайте ему новый пароль.`, 'Сбросить пароль'],
            'toggle-stock-edit': ['Изменить права на сток?', `${name}\n${button.textContent.trim()}.`, 'Подтвердить']
        };
        const drafts = otherDrafts(null);
        const [title, text, label] = descriptions[action];
        const data = new FormData();
        if (password) data.append('password', password);
        const confirmed = await confirmation({ title, text, label, password, danger: action === 'delete' || action === 'reset-password' || (action === 'toggle-active' && !blocked), action: () => post(`/admin/users/${id}/${action}`, data) });
        if (!confirmed) return;
        if (password) await confirmation({ title: 'Пароль изменён', text: 'Скопируйте пароль и передайте сотруднику. Позже его можно будет только заменить.', password, label: 'Готово', cancel: false });
        try {
            ...
            await confirmation({ title: 'Действие выполнено', text: error.message, label: 'Понятно', cancel: false });
        }
    }
    ...
    function initializeCreate() {
        ...
        one('#u-pass-hint').textContent = 'Скопируйте пароль и передайте сотруднику после создания учётной записи.';
    ...
    async function createUser(form) {
        ...
        status.classList.remove('is-error');
        try {
            const labels = { full_name: 'ФИО', google_email: 'Электронная почта', login: 'Логин', password: 'Пароль' };
            for (const [name, label] of Object.entries(labels)) {
                const input = form.elements.namedItem(name);
                if (name !== 'password') input.value = input.value.trim();
                if (!input.value.trim()) throw Object.assign(new Error(`Заполните поле «${label}».`), { field: name });
            }
            if (!one('#u-email').checkValidity()) throw Object.assign(new Error('Укажите корректный адрес электронной почты.'), { field: 'google_email' });
            if (!/^[A-Za-z0-9_.@+\-]+$/.test(one('#u-login').value)) throw Object.assign(new Error('В логине допустимы латиница, цифры и символы . _ @ + −.'), { field: 'login' });
            if (one('#u-pass').value.length < 8) throw Object.assign(new Error('Пароль должен содержать не менее 8 символов.'), { field: 'password' });
            validatePolicy(form);
            ...
            baselines.set(form, formValue(form));
            pending = false;
            await confirmation({ title: 'Сотрудник создан', text: 'Учётная запись готова. Скопируйте пароль и передайте сотруднику вместе с логином.', password, label: 'Готово', cancel: false });
            form.reset();
    ...
    async function requestAction(button) {
        ...
        const url = revoke ? `/admin/access-grants/${button.dataset.grantId}/revoke` : `/admin/access-requests/${actions.dataset.requestId}/decision`;
        const confirmed = await confirmation({
            title: revoke ? 'Отозвать разрешение?' : approve ? 'Разрешить временный доступ?' : 'Отклонить запрос?',
            text: revoke ? 'Сотрудник больше не сможет использовать это временное разрешение.' : approve ? 'Сотрудник получит разрешение на выбранное действие на срок до 7 дней.' : 'Сотрудник не получит временное разрешение на это действие.',
            label: revoke ? 'Отозвать' : approve ? 'Разрешить на 7 дней' : 'Отклонить',
            danger: !approve, action: () => post(url, data)
        });
        if (confirmed) {
            try { await refresh(); toast('Решение сохранено'); }
    ...
    function initialize() {
        ...
        root.addEventListener('click', event => {
            ...
            if (button.dataset.statFilter) {
                ...
                if (filter !== 'requests') { resetFilters(); state.status = filter === 'all' ? '' : filter; syncFilters(); filterUsers(); }
            }
            ...
            if (button.dataset.openUser) { opener = button; openUser(button.dataset.openUser); }
            ...
            if (button.id === 'u-open-create') { opener = button; one('#u-create-overlay').showModal(); one('#u-fio').focus(); }
            if (button.id === 'u-gen') { one('#u-pass').value = generatePassword(); one('#u-pass-hint').textContent = 'Пароль сгенерирован. Нажмите «Копировать», чтобы сохранить его.'; }
            if (button.id === 'u-copy') copyPassword(one('#u-pass').value, one('#u-pass-hint'));
            if (button.id === 'ad-confirm-copy') copyPassword(one('#ad-confirm-password input').value, button);
            if (button.dataset.userAction) userAction(button);
            if (button.matches('.access-request-approve, .access-request-reject, .access-grant-revoke')) requestAction(button);
            if (button.hasAttribute('data-select-stores')) {
                ...
                inputs.forEach(input => { if (!input.disabled) input.checked = checked; });
        ...
        root.addEventListener('input', event => {
            ...
            if (target.id === 'ad-user-search') { state.query = target.value; state.page = 1; filterUsers(); }
            if (target.dataset.tableSearch) { state[target.dataset.tableSearch] = target.value; filterTable(target.dataset.tableSearch); }
            if (target.form) { target.removeAttribute('aria-invalid'); updateDirty(target.form); }
        });
        root.addEventListener('change', event => {
            const target = event.target;
            const filter = { 'ad-filter-role': 'role', 'ad-filter-status': 'status', 'ad-filter-store': 'store', 'ad-sort': 'sort' }[target.id];
            if (filter) { state[filter] = target.value; state.page = 1; filterUsers(); }
            if (target.form) { updatePolicy(target.form); updateDirty(target.form); }
        });
    ...
    window.addEventListener('beforeunload', event => {
        if (pending || dirtyForms().length) { event.preventDefault(); event.returnValue = ''; }
    });
    initialize();
})();
