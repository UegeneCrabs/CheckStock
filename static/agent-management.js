(() => {
  ...
  document.getElementById('agent-local-notice').hidden = location.protocol === 'https:';
  ...
  async function copy(text) {
    try { await navigator.clipboard.writeText(text); tell('Скопировано.'); }
    catch { tell('Браузер не разрешил копирование. Выделите текст и скопируйте вручную.'); }
  }
  document.getElementById('agent-copy-key').onclick = () => copy(token.value);
  document.getElementById('agent-hide-key').onclick = () => { token.value = ''; secret.hidden = true; };
  ...
  window.addEventListener('pagehide', () => { token.value = ''; secret.hidden = true; });
