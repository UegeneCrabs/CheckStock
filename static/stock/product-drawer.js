(function () {
    ...
    var copy = window.CheckStockUI.copyText;
    ...
    function renderWarehouses(items) {
        ...
        items.forEach(function (item) {
            ...
            available.innerHTML = window.CheckStockUI.render('stock/product-drawer/render-warehouses', {
                content: numberFormat.format(item.available || 0),
            });

            var fbs = document.createElement('span');
            fbs.className = 'stock-drawer-warehouse-value stock-drawer-warehouse-value--fbs';
            fbs.innerHTML = window.CheckStockUI.render('stock/product-drawer/render-warehouses-2', {
                content: numberFormat.format(item.fbs || 0),
            });
    ...
    function renderTransit(items) {
        ...
        list.innerHTML = '';
        if (!items.length) {
            list.innerHTML = window.CheckStockUI.render('stock/product-drawer/render-transit');
    ...
    function loadWarehouseDetails(article) {
        ...
        var list = drawerBackdrop.querySelector('[data-stock-drawer-warehouses]');
        list.innerHTML = window.CheckStockUI.render('stock/product-drawer/load-warehouse-details');
        renderTransit([]);
        ...
        fetch(path)
            ...
            .catch(function () {
                if (request !== drawerRequest) return;
                list.innerHTML = window.CheckStockUI.render('stock/product-drawer/load-warehouse-details-2');
    ...
    function openDrawer(row) {
        ...
        setDrawerText(
            '[data-stock-drawer-total]',
            numberFormat.format(numberFromCell(row, '.col-row-total')),
        );
        setDrawerText(
            '[data-stock-drawer-available]',
            numberFormat.format(numberFromCell(row, '.col-ff-available')),
        );
        setDrawerText(
            '[data-stock-drawer-transit-total]',
            numberFormat.format(numberFromCell(row, '.col-transit')),
        );
        ...
        drawerInitial.textContent =
            name && name.textContent.trim() ? name.textContent.trim().charAt(0).toUpperCase() : '?';
    ...
    if (drawerBackdrop)
        drawerBackdrop.addEventListener('mousedown', function (event) {
    ...
    if (drawerOperation)
        drawerOperation.addEventListener('click', function () {
    ...
    if (drawerImage)
        drawerImage.addEventListener('error', function () {
            drawerImage.hidden = true;
        });
