window.createRowEditor = function (options) {
    ...
    function loadSourceStock() {
        ...
        fetch(
            '/stock/' +
                storeSlug +
                '/ff-cell?ff=' +
                encodeURIComponent(src.ff) +
                '&mp=' +
                encodeURIComponent(src.mp),
        )
            .then(function (r) {
                return r.json();
            })
    ...
    function validateRows() {
        ...
        if (dupes.length) {
            problems.push(
                'один и тот же товар в нескольких строках: ' +
                    dupes.join(', ') +
    ...
    function renderSuggest(row, box, code, qty, items) {
        if (!items.length) {
            var src = getSource();
            box.innerHTML = window.CheckStockUI.render('stock/row-editor/render-suggest', {
                content: src.ff ? 'На этом складе такого товара нет' : emptyHint,
            });
        ...
        box.innerHTML = items
            .map(function (it) {
                ...
                return window.CheckStockSuggestions.renderItem(it, it.barcode + ' · есть ' + have);
            })
            .join('');
        box.classList.add('open');
        ...
        code.addEventListener('input', function () {
            ...
            suggestTimer = setTimeout(function () {
                ...
                fetch(url)
                    ...
                    .then(function (d) {
                        renderSuggest(row, box, code, qty, d.items || []);
                    })
                    .catch(function () {
                        closeSuggest(row);
                    });
    ...
    function addRow() {
        ...
        row.innerHTML = window.CheckStockUI.render('stock/row-editor/add-row');
    ...
    function clearPane(pane) {
        pane.querySelectorAll('input').forEach(function (input) {
            if (input.type === 'file') {
                ...
                input.type !== 'number' &&
                !input.classList.contains('mv-code') &&
