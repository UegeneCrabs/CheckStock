(() => {
    ...
    const escapeHtml = window.CheckStockUI.escapeHtml;
    const copyIdentifier = (kind, value, label) =>
        window.CheckStockIdentifierCopy
            ? window.CheckStockIdentifierCopy.html(kind, value, label)
            : escapeHtml(label ?? value);

    const resultHtml = (item) => {
        if (!item.article) {
            return window.CheckStockUI.render('stock/randomizer/result-html', {
                content: item.message || 'Подходящих артикулов не осталось',
            });
        }
        const barcode = item.barcode
            ? window.CheckStockUI.render('common/ui/small-text', {
                  content: copyIdentifier('Баркод', item.barcode, `Баркод ${item.barcode}`),
              })
            : '';
        return window.CheckStockUI.render('stock/randomizer/result-html-2', {
            content: copyIdentifier('Артикул', item.article, item.article),
            content_2: item.name || 'Без названия',
            barcode: barcode,
            content_3: Number(item.ff_quantity || 0).toLocaleString('ru-RU'),
            content_4: Number(item.fbs_quantity || 0).toLocaleString('ru-RU'),
        });
    };

    const updateCard = (item) => {
        const card = root.querySelector(
            `[data-randomizer-card][data-store="${CSS.escape(item.store_slug)}"]`,
        );
        if (!card) return;
