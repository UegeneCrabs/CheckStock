...
Через этот контур работают аутентификация, команды РНП, статусы центра решений, складские перемещения и ставки юнит-экономики. Прикладные сервисы тестируются через моки портов, инфраструктурные репозитории — на отдельной временной БД, HTTP-цепочки — через integration и E2E.
