...
def main() -> int:
    ...
    for store_slug in requested_stores:
        ...
        for marketplace in stock_sheet_export.repository.MARKETPLACES:
            sheet_name = stock_sheet_export._target_sheet_name(settings, marketplace, "fbs_orders").strip()
            if not sheet_name:
                skipped.append(
                    {
            ...
            try:
                spreadsheet_id = stock_sheet_export._spreadsheet_id(settings.spreadsheet_url_for(marketplace))
