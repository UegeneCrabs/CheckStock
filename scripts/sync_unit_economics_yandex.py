...
def main() -> int:
    ...
    parser.add_argument("--source", choices=unit_economics_sync.SOURCES, required=True)
    args = parser.parse_args()
    db.init_db()
    report = {slug: unit_economics_sync.sync_store(slug, args.source) for slug in dict.fromkeys(args.store)}
